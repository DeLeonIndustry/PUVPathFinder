...
$sql_total = "SELECT COUNT(*) FROM accounts WHERE acctype='Commuters'";
$result_total = $conn->query($sql_total);
$total_records = $result_total->fetch_array()[0];
$total_pages = ceil($total_records / $records_per_page);

$sql = "SELECT name, mname, lname, suffix, email, address, dob, contact, idtype, idno, acctype, status FROM accounts WHERE acctype='Commuters'";
                ...
                <div class="iq-card">
                    <div class="iq-card-header d-flex justify-content-between">
                        <div class="iq-header-title">
                            <h4 class="card-title">Commuters List</h4>
                        </div>
                    </div>
                                ...
                                <tbody>
                                    ...
                                    if ($result->num_rows > 0) {
                                        ...
                                        while ($row = $result->fetch_assoc()) {
                                            $fullName = $row["name"] . " " . $row["mname"] . " " . $row["lname"] . " " . $row["suffix"];
                                            $idDetails = $row["idtype"] . " - " . $row["idno"];
                                            $statusClass = $row["status"] == 'Active' ? 'iq-bg-primary' : 'iq-bg-danger';
                                            echo "<tr>
                                                    <td>{$fullName}</td>
                                                    <td>{$row["email"]}</td>
                                                    <td>{$row["address"]}</td>
